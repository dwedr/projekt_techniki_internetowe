...
function validateAnimationSettings(settings) {
  ...
  if (spring_constant <= 0 || mass <= 0 || amplitude < 0 || amplitude > 170 || spring_constant > 100 || mass > 100) {
    return false;
  }

  if (damping_coefficient !== null && (damping_coefficient < 0 || damping_coefficient >= 2 * Math.sqrt(spring_constant * mass))) {
    return false;
  }
...
app.post('/login', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM projekt_techniki.users WHERE username = $1', [username]);
    ...
    await pool.query('INSERT INTO projekt_techniki.refresh_tokens (user_id, token) VALUES ($1, $2)', [user.id, refreshToken]);
...
app.post('/token', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query('SELECT * FROM projekt_techniki.refresh_tokens WHERE token = $1', [refreshToken]);
    if (result.rows.length === 0) return res.status(403).send('Invalid Refresh Token.');
...
app.post('/logout', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query('DELETE FROM projekt_techniki.refresh_tokens WHERE token = $1', [refreshToken]);

    if (result.rowCount === 0) return res.status(404).send('Refresh Token not found.');
...
// Endpoint to save or update user animation settings
app.post('/animation-settings', authenticateToken, async (req, res) => {
  const userId = req.user.userId;

  if (!validateAnimationSettings(req.body)) {
    return res.status(400).send('Invalid animation settings.');
  }
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      return res.status(201).send('Settings saved successfully.');
    }
  } catch (err) {
...
app.get('/animation-settings', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      return res.status(404).send('No saved settings found for this user.');
    }
  } catch (err) {
    console.error('Error retrieving settings:', err.message);
    res.status(500).send('An error occurred while retrieving settings.');
  }
});
